...
exports.init = function(opts) {
	...
	exports.options.version = opts.version || "1.0";
	exports.options.apiRoot = opts.apiRoot || "api";
	exports.options.endpoint = opts.endpoint || "acacia";
...
exports.createApiHandler = function(options, cb) {
	...
	// that allows for curated reponses you back
	if(options.mockDogfood && (!("mockExceptions" in options) || options.mockExceptions.indexOf(Object.keys(options.spec)[0]) === -1)) {
	...
	return function(opts, callback) {
		...
		if(opts.uid)
			opts.uids = utils.flatten([opts.uid]);
		opts.multiID = Array.isArray(opts.uid);
		...
		Object.keys(obj).forEach(function(key) {
			if(verbs.indexOf(key.toLowerCase()) > -1 || typeof obj[key] === "function" || Array.isArray(obj[key])) {
				...
				if(key[0] == ":") {
					...
					access[key.slice(1).replace(/[\*\+\.\?]/g, "")][verb] = exports.createApiHandler(options, func);
				}
