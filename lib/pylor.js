...
exports.resolveRole = function(role) {
	// Resolve the roles into a permissions set
	var set = role.permissions;
...
exports.hasAccess = function(checks, user, noExtensions) {
	...
	var validateCheck = function(check) {
		...
		// if yes === 1, it means we have permission with a wildcard, which has second highest priority
		// Negation has the next highest priority
		return yes === 1 && no !== 1 || !no && yes;
	};
...
exports.matchGrantValues = function(grant, user, match) {
	if(!grant)
	...
	var vals = exports.getGrantValues(grant, user);
...
exports.getGrantValues = function(grant, user, noExtensions) {
	if(!grant)
	...
	if(!noExtensions && g in grantExtensions) {
		for(var i = 0; i < grantExtensions[g].length; i++) {
			if(grantExtensions[g][i](user) === null)
				return null;
			ret = ret.concat(grantExtensions[g][i](user));
		}
	}
...
exports.checkPermission = function(req, res, next) {
	...
	res.status(401).json({info: "Access denied", detail: "User is not logged in"});
};
